...
export default function LoginPage() {
  ...
  // Strongly remove any top-padding/margin from html/body/#root while auth page is mounted
  useLayoutEffect(() => {
    ...
    // Apply forced zero padding/margin
    ...
    // also add a utility class (in case your CSS uses !important or selectors)
    body.classList.add("auth-no-pad");

    return () => {
      // restore previous inline styles (or clear if none)
  ...
  return (
    <>
      <style>{`
        ...
        label { display:block; margin-bottom:6px; font-size:14px; font-weight:600; color:#fff; }
        ...
        .lp-btn:disabled { opacity:.8; cursor:not-allowed; }
      ...
      <div className="auth-root" role="main" aria-label="Authentication page">
        <div className="lp-card" aria-labelledby="login-heading">
          <h2 id="login-heading" className="lp-title">Welcome Back</h2>
          <div className="lp-sub">Sign in to continue</div>
          ...
          <form onSubmit={handleLogin} noValidate>
            <div className="lp-field">
              ...
              <input id="username" className="lp-input" type="text" value={username} placeholder="Enter username" onChange={(e) => setUsername(e.target.value)} autoComplete="username" />
            </div>

            <div className="lp-field">
              <label htmlFor="password">Password</label>
              <div className="lp-password-wrapper">
                <input id="password" className="lp-input" type={showPassword ? "text" : "password"} value={password} placeholder="Enter password" onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" />
            ...
            <button className="lp-btn" type="submit" disabled={loading}>{loading ? "Signing in..." : "Login"}</button>
